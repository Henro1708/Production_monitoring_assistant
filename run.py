...
if __name__ == "__main__": #this is necessary to make sure the thing exists
    ...
    lunchP = ["12:00", "20:00", "04:15"]   
    breakP = ["09:00","17:00","01:45"]   # CHECK THESE TIMES
    ...
    lunchArraysAlt =[lunchP]
    breakArraysAlt = [breakP]
    endTimeArraysAlt = [endTimeP]
    firstHourArraysAlt = [firstHoursP]
    ...
    for i in range(len(alternativeStations)):
        procA = Process(target = mainAlt, args = (alternativeStations[i],endTimeArraysAlt[i],breakArraysAlt[i] ,lunchArraysAlt[i],firstHourArraysAlt[i], ))
