...
def afterAnHour(endTime): # checks the time a part takes to be made after the first hour
    t1 = time.time()
    t2 = time.time()
...
def checkPart(endTime):    # main function that checks when a part is made and how long it took
    t = time.time()
    t2 = time.time()
    timeNow = datetime.now()
...
# MAIN JL LONG ## MAIN JL LONG ## MAIN JL LONG ## MAIN JL LONG ## MAIN JL LONG ## MAIN JL LONG ## MAIN JL LONG ## MAIN JL LONG ## MAIN JL LONG #
...
def JLLongFun():
    ...
    while True:                         # BEGINNING OF SHIFT ## BEGINNING OF SHIFT ## BEGINNING OF SHIFT ## BEGINNING OF SHIFT #
        ...
        if happenedInFirst == True:   # Tests for the first hour (anytime in the first hour counts as a full hour)
            ...
            resAfterAnHour,timeUsed = afterAnHour(endTime)   #if not in first hour, checks for the next ones
        ...
        while now.strftime("%H:%M") !=endTime:   #START CHECKING EVERY MOMENT
            if endTime == '0':
                break
            pMde, timePerPart = checkPart(endTime) #calls function to check parts
